...
class AsyncServiceHelper
{
    public static boolean initOpenCV(String Version, final Context AppContext, final LoaderCallbackInterface Callback)
    {
        ...
        bservice = Build.VERSION.SDK_INT >= 21 ?
                AppContext.bindService(createExplicitFromImplicitIntent(AppContext, new Intent("org.opencv.engine.BIND")), helper.mServiceConnection, Context.BIND_AUTO_CREATE) :
                AppContext.bindService(new Intent("org.opencv.engine.BIND"), helper.mServiceConnection, Context.BIND_AUTO_CREATE);

        if (bservice) { return true; }
